...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        # VARIABLES        
        *)
            if [[ "${1}" == "$(echo ${1} | grep -E '^[0-9A-Z_]+$')" ]] && [[ ! -z "$2" ]]; then
                export JSONSV_${1}="${2}"
                shift 2
            else
...
if [[ "${COMMAND}" == "LIST" ]]; then
    if [[ -f "${TEMPLATE_PATH}/${REQUEST_NAME}.json" ]]; then
        FIELDS=$(grep -E '#[0-9A-Z_]+#' ${TEMPLATE_PATH}/${REQUEST_NAME}.json | sed -Ee 's/^.*#([0-9A-Z_]+)#.*$/\1/')
...
echo sed ${FIELD_REPLACE} ${TEMPLATE_PATH}/${REQUEST_NAME}.json | bash | grep -E -v "#[0-9A-Z_]+#" >${TMP_FILE}
